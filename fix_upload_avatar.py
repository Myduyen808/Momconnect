# Script sửa avatar trong home.html
# Thay thế avatar_url bằng url_for để đồng bộ với header

import os
import re

def fix_home_avatars():
    """Sửa tất cả avatar trong home.html"""
    
    print("🔧 SỬA AVATAR TRONG HOME.HTML")
    print("="*50)
    
    home_file = 'templates/home.html'
    
    if not os.path.exists(home_file):
        print(f"❌ Không tìm thấy file {home_file}")
        return False
    ...
    replacements = [
        ...
        # 4. Avatar bạn bè
        (
            r'src="\{\{\s*friend\.avatar_url\s*\}\}"',
            'src="{{ url_for(\'static\', filename=friend.avatar or \'images/default-avatar.png\') }}"'
        ),
        # 5. Avatar gợi ý kết bạn
        (
            r'src="\{\{\s*user\.avatar_url\s*\}\}"',
            'src="{{ url_for(\'static\', filename=user.avatar or \'images/default-avatar.png\') }}"'
        )
    ]
    ...
    if changes_made:
        print(f"✅ Đã sửa {len(changes_made)} avatar trong home.html")
        for change in changes_made:
            print(f"  {change}")
        return True
    else:
        print("❌ Không tìm thấy avatar_url cần sửa")
        return False

def create_fixed_home():
    """Tạo file home.html đã sửa sẵn"""
    
    print("\n📝 TẠO FILE HOME.HTML ĐÃ SỬA")
    print("-"*40)
    
    # Nội dung file home.html đã sửa
    fixed_home_content = '''{% extends "base.html" %}
...
<div class="grid lg:grid-cols-3 gap-8 max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <div class="bg-white rounded-3xl shadow-xl overflow-hidden hover:shadow-2xl transition-all">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div id="friend-button-{{ post.author.id }}">
                        ...
                        <button onclick="handleFriendAction('{{ post.author.id }}', 'unfriend', this)"
                            ...
                            <i class="fas fa-user-minus mr-2"></i>Bạn bè
                        </button>
            ...
            <div class="p-6 pt-4">
                    ...
                    <div class="flex items-center gap-8">
                        ...
                        <button onclick="focusComment({{ post.id }})" class="flex items-center gap-2 group">
                            ...
                            <span class="font-bold text-blue-700" id="comment-count-{{ post.id }}">{{
                                post.comments_count }} Bình luận</span>
                        </button>
                        <button onclick="toggleShareMenu({{ post.id }})" class="flex items-center gap-2 group relative">
                            ...
                            <span class="font-bold text-green-700">Chia sẻ</span>
                        </button>
                    </div>
                ...
                <div id="share-menu-{{ post.id }}"
                    ...
                    <p class="text-sm font-bold text-gray-700 mb-3">Chia sẻ bài viết đến:</p>
                ...
                <div class="mt-6 flex gap-4">
                    ...
                    <input type="text" placeholder="Viết bình luận..." id="quick-comment-{{ post.id }}"
        ...
        <div class="text-center py-20 bg-white rounded-3xl shadow-xl">
            ...
            <p class="text-2xl text-gray-500 font-medium">Chưa có bài viết nào</p>
    ...
    <div class="space-y-8">
        ...
        <div class="bg-gradient-to-br from-pink-400 to-purple-600 p-8 rounded-3xl text-white shadow-2xl">
            <div class="text-center">
                ...
                <p class="mt-2 text-pink-100">{{ current_user.points }} điểm tích lũy</p>
        ...
        <div class="bg-white rounded-3xl shadow-xl p-6">
            <h3 class="text-xl font-bold mb-4">Bạn bè ({{ friends|length }})</h3>
            {% if friends %}
            <div class="space-y-3">
            ...
            {% else %}
            <p class="text-gray-500 text-center py-8">Chưa có bạn bè</p>
            {% endif %}
        </div>

        <!-- Gợi ý kết bạn -->
        <div class="bg-white rounded-3xl shadow-xl p-6">
            <h3 class="text-xl font-bold mb-4">Gợi ý kết bạn</h3>
            {% if suggested_users %}
            <div class="space-y-3">
            ...
            {% else %}
            <p class="text-gray-500 text-center py-8">Không có gợi ý</p>
            {% endif %}
        </div>
    </div>
</div>
...
<script>
    // Hàm xử lý các hành động kết bạn
    async function handleFriendAction(userId, action, button) {
        try {
            ...
            if (result.success) {
                ...
                showToast(result.error || 'Đã có lỗi xảy ra', false);
            }
        } catch (error) {
    ...
    // Cập nhật giao diện nút kết bạn
    ...
    // Các hàm khác giữ nguyên từ code cũ
    async function likePost(postId) {
        try {
            ...
            if (res.ok) {
                ...
                showToast('Không thể thích bài viết', false);
            }
        } catch (error) {
            console.error('Error:', error);
            showToast('Lỗi kết nối', false);
    ...
    async function quickComment(postId) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                showToast('Không thể gửi bình luận', false);
            }
        } catch (error) {
            console.error('Error:', error);
            showToast('Lỗi kết nối', false);
    ...
    function shareFacebook(postId) {
        ...
        showToast('Đang mở Facebook để chia sẻ...');
    }
    ...
    function shareTwitter(postId) {
        const url = window.location.origin + '/post/' + postId;
        const shareUrl = 'https://twitter.com/intent/tweet?url=' + encodeURIComponent(url) + '&text=' + encodeURIComponent('Xem bài viết này!');
        window.open(shareUrl, '_blank', 'width=600,height=400');
        showToast('Đang mở Twitter/X để chia sẻ...');
    }

    function shareInstagram(postId) {
        showToast('Instagram không hỗ trợ chia sẻ link trực tiếp. Vui lòng copy link và dán vào story/bio!', false);
        copyLink(postId);
    }

    function shareTelegram(postId) {
        const url = window.location.origin + '/post/' + postId;
        const shareUrl = 'https://t.me/share/url?url=' + encodeURIComponent(url) + '&text=' + encodeURIComponent('Xem bài viết này!');
        window.open(shareUrl, '_blank', 'width=600,height=400');
    ...
    async function reportPost(postId) {
        ...
        try {
            ...
            const data = await res.json();
            showToast(data.success ? 'Đã gửi báo cáo thành công!' : data.error || 'Lỗi khi báo cáo', !!data.success);
        } catch (error) {
            console.error('Error:', error);
            showToast('Lỗi kết nối', false);
        }
    ...
    async function deletePost(postId) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                showToast('Không thể xóa bài viết', false);
            }
        } catch (error) {
            console.error('Error:', error);
            showToast('Lỗi kết nối', false);
    ...
    console.log('🎉 Home page with Facebook-style friend system loaded!');
</script>
{% endblock %}'''
    
    with open('home_fixed.html', 'w', encoding='utf-8') as f:
        f.write(fixed_home_content)
    
    print("✅ Đã tạo file home_fixed.html với tất cả avatar đã sửa")
    return True

if __name__ == "__main__":
    print("🚀 SỬA TẤT CẢ AVATAR TRONG HOME.HTML")
    print("="*50)
    ...
    print("1. Sao chép nội dung file home_fixed.html vào templates/home.html")
    print("2. Hoặc sửa thủ công các dòng avatar_url:")
    print("   - post.author.avatar_url")
    print("   - current_user.avatar_url")
    print("   - friend.avatar_url")
    print("   - user.avatar_url")
    print("3. Thay thế bằng: url_for('static', filename=...)")
    print("4. Khởi động lại ứng dụng và test lại")