...
class User(UserMixin, db.Model):
    ...
    # XÓA CÁC RELATIONSHIP NÀY - đã định nghĩa ở Follow/FriendRequest
    # following, followers, friend_requests_sent, friend_requests_received
    ...
    @property
    def friends(self):
        sent = FriendRequest.query.filter_by(sender_id=self.id).all()
        received = FriendRequest.query.filter_by(recipient_id=self.id).all()

        users = []
        for f in sent:
            users.append(f.recipient)
        for f in received:
            users.append(f.sender)
        return users
